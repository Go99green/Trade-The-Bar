...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-bone/[0.08]" style={{ backgroundColor: "#1C2416" }}>
      <div className="max-w-7xl mx-auto px-5 md:px-10 h-16 flex items-center justify-between">
          ...
          {navLinks.map(({ href, label }) => (
            <Link
              ...
              className={`nav-link text-[13px] tracking-widest uppercase font-sans font-medium transition-colors ${
                pathname === href ? "text-rust" : "text-bone/50 hover:text-bone"
              }`}
            >
              {label}
            </Link>
        ...
        <div className="flex items-center gap-4">
          <Link
            href="/cart"
            className="text-bone/60 hover:text-rust transition-colors"
            aria-label="Cart"
          >
            <CartIcon />
          </Link>
          {/* Hamburger */}
          <button
            className="md:hidden text-bone/60 hover:text-rust transition-colors"
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden border-t border-bone/10" style={{ backgroundColor: "#1C2416" }}>
